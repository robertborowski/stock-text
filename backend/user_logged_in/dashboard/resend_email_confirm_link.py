...
resend_email_confirm_link = Blueprint("resend_email_confirm_link", __name__, static_folder="static", template_folder="templates")

@resend_email_confirm_link.before_request
def before_request():
...
@resend_email_confirm_link.route("/resend_email_confirm_link", methods=["POST", "GET"])
def resend_email_confirm_link_function():
  ...
  if session and session.get('logged_in_user_email') != 'none':
    ...
    confirm_email_token = create_confirm_token_function(user_email, os.environ.get('URL_SAFE_SERIALIZER_SECRET_KEY_EMAIL'), os.environ.get('URL_SAFE_SERIALIZER_SECRET_SALT_EMAIL'))
